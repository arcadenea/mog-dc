...
void GameCycle(BYTE *screen,int dx,int dy)
{
	...
	case 0:	{
				...
				if (SUBSTATE==0) {
					init_paths();
#ifdef DREAMCAST					
					if (!cargar_configuracion("/vmu/mog.cfg")) {
						configuracion_por_defecto();
						guardar_configuracion("/vmu/mog.cfg");
					} /* if */ 
#else
...
#ifdef DREAMCAST
            tile_print("PUSH A BUTTON",TILE_SIZE_X*13,TILE_SIZE_Y*19,screen,dx,dy);
			sprintf(tmp,"L CHANGES GRAPHIC SET: %s",g_path+9);
#else
...
#ifdef DREAMCAST			
			sprintf(tmp,"R CHANGES SOUND SET: %s",s_path+6);
#else			
...
#ifdef DREAMCAST			
			tile_print("PRESS Y TO REDEFINE THE KEYS",TILE_SIZE_X*6,TILE_SIZE_Y*23,screen,dx,dy);
#else
...
//				set_palete((BYTE *)tiles_bmp->get_palete());
			...
			if (!gpause) {
				...
				if (typed_word_p("zeus") && map==0 && map_x==5 && map_y==12 && gpause) {
					ZEUS_password=true;
				} /* if */ 
				if (pause_state<0) {
			...
			if (keyboard[ITEM_KEY] && !old_keyboard[ITEM_KEY] && player_energy[character]>0 && !gpause && to_enter_cut==0) {
				Sound_play(S_F1);
		...
		case 15:
			{
				...
				if (fp!=0) {
#ifdef DREAMCAST                          
					tile_print("PRESS A TO START",TILE_SIZE_X*9,TILE_SIZE_Y*8,screen,dx,dy);
#else
